...
public class SpringUtils
{
    ...
    {
        String key = System.currentTimeMillis() + RandomUtils.randomIntStr(7);

        key = DigestUtils.md5LowerCase(key, ProjectCst.$_CHARSET);
     ...
     *
     * @param userid     用户编号
     * @param sessionKey 用户身份信息
     * @param appKey     APP 密钥
     * @param token
     * @return true 表示签名匹配成功；false 表示签名匹配失败
     */
    public static boolean validUserToken(int userid, String sessionKey, String appKey, String token)
    {
        String localToken = DigestUtils.md5LowerCase(userid + sessionKey + appKey, ProjectCst.$_CHARSET);

        return localToken.equalsIgnoreCase(token);
    }
    ...
    public static String appendUrl(String subUrl, String preUrl)
    {
        ...
        if (subUrl.startsWith("http://") || subUrl.startsWith("HTTP://")) return subUrl;

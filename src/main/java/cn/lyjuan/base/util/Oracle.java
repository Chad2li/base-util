...
public class Oracle
{
    ...
    private String sequenceName;// insert操作中序列的名字
     ...
     * @return
     */
    public Oracle pool(final String pool)
    {
        this.pool = pool;
        return this;
    }
     ...
     * @return
     */
    public Oracle table(String table)
    {
        this.table = table;
        return this;
    }
     ...
     * @return
     */
    public Oracle pageNum(int pageNum)
    {
     ...
     * @return
     */
    public Oracle pageSize(int pageSize)
    {
     ...
     * @return
     */
    public Oracle orderBy(String column, OrderBySort orderBySort)
    {
        this.orderBy = column;
     ...
     * @return
     */
    public Oracle orderBy(String orderBy)
    {
     ...
     * @return
     */
    public Oracle fields(String... field)
    {
     ...
     * @return
     */
    public Oracle sequence(String idColumn, String sequenceName)
    {
    ...
    /**
     * insert操作中不使用bean方式需要设置对应的列和值
     *
     * @param column
     * @param value
     * @return
     */
    public Oracle insertField(String column, Object value)
    {
    ...
    /**
     * update操作中不使用bean方式需要设置对应的列和值
     *
     * @param column
     * @param value
     * @return
     */
    public Oracle updateField(String column, Object value)
    {
        return this.insertField(column, value);
    }


    // public JDBCUtilForOracle setInsertOrUpdateFields(Map<String,Object> map){
    // this.insertOrUpdateFields = new ArrayList<ColumnAndValue>();
    // for (Entry<String, Object> entry:map.entrySet()){
    // ColumnAndValue cav = new ColumnAndValue();
    // cav.setColumn(entry.getKey());
    // cav.setValue(entry.getValue());
    // this.insertOrUpdateFields.add(cav);
    // }
    // return this;
    // }
     ...
     * @return
     */
    public Oracle where(String column, String operator, Object value)
    {
        if (value == null || "".equals(value))
        {
            throw new RuntimeException("where条件的值不能为空！");
        }
        if (this.where == null)
        {
     ...
     * @return
     */
    public Oracle whereNull(String column, ColumnNull cn)
    {
     ...
     * @param showSql
     */
    public Oracle showSql(boolean showSql)
    {
     ...
     * @return
     */
    public Oracle annotation(boolean annotation)
    {
    ...
    /**
     * insert操作,将参数对象的值保存到数据库并且返回插入的id，必须设置insertSequence
     *
     * @param <T>
     * @param t
     * @return 插入的id
    ...
    {
        if (this.idColumn == null || this.idColumn.trim().length() == 0)
        {
    ...
    public int insert()
    {
        if (this.insertOrUpdateFields == null)
        {
            throw new RuntimeException("必须设置需要插入的字段！");
        }
        ...
        try
        {
            ...
            {
                if (this.idColumn != null && this.idColumn.equals(cav.getColumn()))
                {
                    continue;
                }
        ...
        {
            PoolDBUtils.close(ps);
            PoolDBUtils.close(conn);
        }
        return num;
    }
    ...
    public int insertReturnID()
    {
        ...
        String            sql  = this.getInsertReturnIDSql();
        Connection        conn = null;
        ...
        try
        {
            ...
            ps.registerOutParameter(i, Types.NUMERIC);
            ps.execute();
            id = ps.getInt(i);
        } catch (SQLException e)
        {
     ...
     * @return
     */
    public String getCustomPageSql(String sql, String orderby)
    {
        StringBuilder sbSql = new StringBuilder();
        sbSql.append("SELECT * FROM (\n").append("  SELECT b1.*, ROWNUM rnum FROM (\n").append(sql)
                .append("  ) b1 WHERE ROWNUM <= ?\n").append(") WHERE rnum > ?\n");
     ...
     * @param c
     * @param sql
     * @param param sql中问号对应的值 最后两个为 pageNum（当前页） 和 pageSize（页大小）
     * @return
     */
    public <T> List<T> selectSqlPage(Class<T> c, String sql, String orderby, Object... param)
    {
        this.setFields(c);
        sql = getCustomPageSql(sql, orderby);
        Connection        conn = null;
        PreparedStatement ps   = null;
        ResultSet         rs   = null;
        List<T>           list = new ArrayList<T>();
        ...
        if (param != null && param.length > 0)
            vals = new ArrayList<Object>(Arrays.asList(param));
        else
            vals = new ArrayList<Object>();

        vals.add(this.pageNum * this.pageSize);
        vals.add((this.pageNum - 1) * this.pageSize);
     ...
     * @return
     */
    public <T> List<T> selectSql(Class<T> c, String sql, Object... param)
    {
        this.setFields(c);
        Connection        conn = null;
        PreparedStatement ps   = null;
     ...
     * @return
     */
    public List<Map<String, Object>> selectSqlMapPage(String sql, String orderby, Object... param)
    {
        sql = getCustomPageSql(sql, orderby);
    ...
    private String getInsertSql()
    {
        ...
        if (this.idColumn != null)
        {
            sql.append(this.sequenceName).append(".nextval,");
        }
        for (ColumnAndValue cav : this.insertOrUpdateFields)
        {
            if (this.idColumn != null && this.idColumn.equals(cav.getColumn()))
    ...
    private String getInsertReturnIDSql()
    {
        if (this.table == null || this.table.trim().length() == 0)
        {
            throw new RuntimeException(
        ...
        StringBuffer sql = new StringBuffer();
        sql.append("begin insert into ").append(this.table).append("(");
        if (this.idColumn != null)
        {
            sql.append(this.idColumn).append(",");
        }
        ...
        sql.setCharAt(sql.length() - 1, ')');
        sql.append(" return ").append(this.idColumn).append(" into ?;end; ");
        return sql.toString();
    }
    ...
    private String getSelectSql()
    {
        ...
        if (this.pageNum > 0 && this.pageSize > 0)
        {
            sql.insert(0, "select * from (select a.*,rownum rn from (");
    ...
    public String selectSequence(String seqName)
    {
        ...
        try
        {
            ...
            if (rs.next())
            {
                seqVal = rs.getBigDecimal(1).toString();
            }
        } catch (SQLException e)
        {
            throw new RuntimeException("selectArray->数据库异常:" + e.getMessage(), e);
     ...
     * 获得连接
     *
     * @throws java.sql.SQLException added by chenhang 2013-01-14
     */
    private Connection getConn() throws SQLException
    {
        if (this.pool != null && this.pool.length() > 0)
        {
            return PoolDBUtils.getConnection(this.pool);
        }
        return PoolDBUtils.getConnection();
    }
